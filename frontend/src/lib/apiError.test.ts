...
describe('api error parsing', () => {
  test('preserves status, code, and detail precedence', async () => {
    const error = await parseErrorResponse(jsonResponse({
  ...
  test('falls back through message, title, and missing code', async () => {
    const messageError = await parseErrorResponse(jsonResponse({ message: 'Payload message' }), 'Fallback')
    const titleError = await parseErrorResponse(jsonResponse({ title: 'Problem title' }), 'Fallback')
  ...
  test('parseJsonResponse returns parsed 2xx bodies', async () => {
    await expect(parseJsonResponse<{ ok: true }>(jsonResponse({ ok: true }, 200), 'Fallback'))
      .resolves.toEqual({ ok: true })
  })

  test('parseJsonResponse throws ApiError for non-2xx bodies', async () => {
    try {
      await parseJsonResponse(jsonResponse({ detail: 'Cart disappeared', code: 'not_found' }, 404), 'Fallback')
