...
test('turns delivery into a branded, guided Meant match', () => {
  ...
  expect(markup).toContain('data-stage="delivery"')
  expect(markup).toContain('mt-checkout-step delivery active')
...
test('moves the story to a secure final step after delivery details are set', () => {
  ...
  expect(markup).toContain('class="mt-checkout-journey compact stage-secure"')
  expect(markup).toContain('mt-checkout-step delivery done')
  expect(markup).toContain('mt-checkout-step secure active')
})
