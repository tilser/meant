...
import com.meant.api.provider.shopify.catalog.dto.ShopifyGlobalCatalogResponse.CapabilityVersion;
...
public class ShopifyGlobalCatalogResponseParser {
    ...
    public ParsedResponse parse(
        ...
        if (operation == CatalogSourceOperation.GET_PRODUCT
                && !"error".equalsIgnoreCase(payload.ucp().status())
                && payload.resolvedProducts().size() != 1) {
    ...
    private void validateEnvelope(ShopifyGlobalCatalogResponse response, CapabilityId requiredCapability) {
        ...
        requireCapability(
                response,
                ShopifyGlobalCatalogExtensionCapability.ID.value(),
