...
public class CheckoutCompletionStateStore {
    ...
    public CheckoutCompletionState markCompleted(@NotNull @Valid StartCheckoutCompletionCommand command) {
        ...
        if (state.getStatus() != CheckoutCompletionStatus.COMPLETION_IN_FLIGHT) {
            throw new CheckoutSafetyException("Checkout completion is not in flight: " + command.checkoutId());
        }
    ...
    public CheckoutCompletionState markCompletedFromRemoteStatus(@NotNull @Valid StartCheckoutCompletionCommand command) {
        ...
        if (state.getStatus() != CheckoutCompletionStatus.AUTHORIZED_TO_COMPLETE
            ...
            throw new CheckoutSafetyException(
    ...
    public CheckoutCompletionState markCanceled(@NotNull @Valid StartCheckoutCompletionCommand command) {
        ...
        if (state.getStatus() != CheckoutCompletionStatus.CANCELED) {
            throw new CheckoutSafetyException("Checkout cancellation was not reserved: " + command.checkoutId());
        }
    ...
    private CheckoutCompletionState authorizeExisting(
        ...
        if (existing.getStatus() == CheckoutCompletionStatus.COMPLETION_IN_FLIGHT
            ...
            throw new CheckoutSafetyException("Checkout completion cannot be re-authorized after completion starts");
        }
