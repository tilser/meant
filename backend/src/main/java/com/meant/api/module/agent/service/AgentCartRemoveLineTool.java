...
public class AgentCartRemoveLineTool implements AgentTool {

    private static final AgentToolDescriptor DESCRIPTOR = new AgentToolDescriptor(
            "remove_cart_line",
            "Remove an owned cart line using its stable Meant cart-line ID.",
            """
    ...
    public AgentToolExecutionResult execute(AgentToolExecutionContext context, String argumentsJson) {
        AgentCartToolArguments.RemoveLine arguments = support.arguments(
                argumentsJson, AgentCartToolArguments.RemoveLine.class);
        AgentCartResult result = AgentCartResult.success(List.of(support.removeLine(context, arguments)));
        String resultJson = support.json(result);
        return new AgentToolExecutionResult(
                resultJson, "Removed the cart line.", support.artifacts(result),
