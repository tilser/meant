...
public class AgentMissionUpdateTool implements AgentTool {

    private static final AgentToolDescriptor DESCRIPTOR = new AgentToolDescriptor(
            "update_shopping_mission",
            "Update an owned mission. Omitted fields are preserved; supplied empty planning arrays are cleared. Readiness is computed from coverage and commerce references remain server-managed.",
            """
                    {"type":"object","additionalProperties":false,"required":["missionId"],"properties":{
                      "missionId":{"type":"string","format":"uuid"},
                      "goal":{"type":"string","minLength":1,"maxLength":500},
                      "status":{"type":"string","enum":["ACTIVE","READY","CHECKOUT_PREPARED","COMPLETED","CANCELLED"]},
    ...
    public AgentToolExecutionResult execute(AgentToolExecutionContext context, String argumentsJson) {
        AgentMissionToolArguments.Update arguments = support.arguments(
                argumentsJson, AgentMissionToolArguments.Update.class);
        AgentMissionDetails mission = support.update(context, arguments);
