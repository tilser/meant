...
import com.meant.api.module.checkout.repository.BuyerConsentRepository;
import com.meant.api.module.checkout.service.command.CreateBuyerConsentCommand;
...
public class BuyerConsentService {
    ...
    public BuyerConsentArtifact findArtifact(UUID consentId, UUID userId) {
        BuyerConsent consent = repository.findByIdAndUserId(consentId, userId)
                .orElseThrow(() -> new CheckoutSafetyException("Buyer consent was not found: " + consentId));
        if (!consent.getExpiresAt().isAfter(Instant.now())) {
            throw new CheckoutSafetyException("Buyer consent has expired: " + consentId);
        }
        return artifact(consent);
    }
    ...
    private String toJson(List<BuyerConsentArtifact.LineItem> value) {
        try {
            ...
            throw new CheckoutSafetyException("Buyer consent artifact could not be serialized", exception);
        }
    }
    ...
    private <T> T fromJson(String value, TypeReference<T> type) {
        try {
            ...
            throw new CheckoutSafetyException("Buyer consent artifact could not be parsed", exception);
        }
    }
