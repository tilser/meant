...
@Service
public class ReviewOutboundUrlValidator {

    private final AddressResolver addressResolver;

    public ReviewOutboundUrlValidator() {
        this(host -> Arrays.asList(InetAddress.getAllByName(host)));
    }

    private ReviewOutboundUrlValidator(AddressResolver addressResolver) {
        this.addressResolver = addressResolver;
    }

    static ReviewOutboundUrlValidator withResolver(AddressResolver addressResolver) {
        return new ReviewOutboundUrlValidator(addressResolver);
    }
    ...
    private String validateAbsoluteHttpsUrlAndNormalizeHost(URI uri) {
        if (uri == null || !uri.isAbsolute() || uri.isOpaque()) {
            throw new ReviewException("Review outbound URL must be absolute");
        }
        if (!"https".equalsIgnoreCase(uri.getScheme())) {
            throw new ReviewException("Review outbound URL must use https");
        }
        if (uri.getHost() == null || uri.getHost().isBlank()) {
            throw new ReviewException("Review outbound URL must include a host");
        }
        if (uri.getUserInfo() != null) {
            throw new ReviewException("Review outbound URL must not include user info");
        }
    ...
    private void resolveAndValidatePublicHost(String normalizedHost) {
        ...
        try {
            ...
            throw new ReviewException("Review outbound URL host could not be resolved", exception);
        }
    ...
    void validatePublicAddresses(List<InetAddress> addresses) {
        if (addresses == null || addresses.isEmpty()) {
            throw new ReviewException("Review outbound URL host did not resolve to an address");
        }
        addresses.forEach(address -> {
            if (!isPublicAddress(address)) {
                throw new ReviewException(
                        "Review outbound URL resolved to a non-public address: " + address.getHostAddress()
                );
            }
        });
    }
    ...
    private String normalizeHost(String host) {
        try {
            return IDN.toASCII(host.trim(), IDN.USE_STD3_ASCII_RULES).toLowerCase(Locale.ROOT);
        } catch (IllegalArgumentException exception) {
            throw new ReviewException("Review outbound URL host is invalid", exception);
        }
    }
